...
func initWallets() {
	...
	for i := range wallets {
		key := func(s string, i int) string {
			return fmt.Sprintf("wallets.%d.%s", i, s)
		}

		wallets[i] = &walletInfo{
			Seed:     viper.GetString(key("seed", i)),
			Salt:     viper.GetString(key("salt", i)),
			IsBip39:  viper.GetBool(key("isbip39", i)),
			IsLedger: viper.GetBool(key("isledger", i)),
			Accounts: make(map[string]uint32),
		}

		for k, v := range viper.GetStringMap(key("accounts", i)) {
...
func initNewWallet(ctx context.Context) *walletInfo {
	...
	if seed == "" {
		entropy, err := bip39.NewEntropy(constants.KeyNumBits)
		fatalIf(err)
...
func (wi *walletInfo) initAccounts(ctx context.Context) {
	err := wi.w.ScanForAccounts(ctx)
	fatalIf(err)
