...
func workGenerate(data, difficulty []byte) (work []byte, err error) {
	...
	if work, err = gpuWorkGenerate(data, target); err != nil {
...
func gpuWorkGenerate(data []byte, target uint64) (work []byte, err error) {
	...
	for _, platform := range platforms {
		...
		return gpuWork(data, target, devices)
	}
	return nil, errors.New("no gpu found")
}

func gpuWork(data []byte, target uint64, devices []*cl.Device) (work []byte, err error) {
	work = make([]byte, 8)
...
func cpuWorkGenerate(data []byte, target uint64) (work []byte, err error) {
	n := runtime.NumCPU()
	ch := make(chan []byte, n)
